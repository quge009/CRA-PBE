...
public class DutchFPSB3StrategyWriterRound2 {
    ...
    public String write(Histories<Double> hist)  {
    	DutchFPSB3Histories histories = (DutchFPSB3Histories) hist;
    	StringBuilder builder = new StringBuilder();
    	int beliefsNr=histories.getnHistories();
    	for(int i=0;i<beliefsNr;i++) {
    		builder.append(String.format("%5.4f", histories.getBelief(i, true)[0]));	
    		builder.append(" ");
    		builder.append(String.format("%5.4f", histories.getBelief(i, true)[1]));
    		builder.append("\n");
    	}
