...
public class BNEAlgorithm<Value, Bid> {
	public static class Result<Value, Bid> {
		...
		public List<Utility<Value>> equilibriumUtilities;
		public Result(double epsilon, List<Strategy<Value, Bid>> equilibriumStrategies, List<Utility<Value>> equilibriumUtilities) {
			this.epsilon = epsilon;
			this.equilibriumStrategies = equilibriumStrategies;
			this.equilibriumUtilities=equilibriumUtilities;
		}
	}
	...
	private double playOneRound(List<Strategy<Value, Bid>> strategies,List<Utility<Value>> utilities, BRCalculator<Value, Bid> brc) {
		...
		Map<Integer, Utility<Value>> bestResponseMapUtil = new HashMap<>();
		...
		for (int i=0; i<nBidders; i++) {
			if (updateBidder[i]) {
				...
				utilities.set(i, bestResponseMapUtil.get(canonicalBidders[i]));
			}
		}
	...
	private Result<Value, Bid> verify(List<Strategy<Value, Bid>> strategies, List<Utility<Value>> utilities, Verifier<Value, Bid> verifier) {
		double highestEpsilon = 0.0;
		...
		return new Result<>(highestEpsilon, sConverted, utilities);
	}
	...
	public Result<Value, Bid> run() {
		...
		List<Utility<Value>> utilities = new ArrayList<>();
		for (int i=0; i<nBidders; i++) {
			utilities.add(null);
		}
		...
		while (iteration <= maxIters) {
			...
			while (iteration <= maxIters) {
				...
				highestEpsilon = playOneRound(strategies, utilities,brc);
				context.advanceRngs();
			...
			if (iteration > maxIters) {
				//return new Result<>(Double.POSITIVE_INFINITY, strategies, utilities); //TODO change back for verifier
		...
		Result<Value, Bid> result = verify(strategies, utilities, context.verifier);
