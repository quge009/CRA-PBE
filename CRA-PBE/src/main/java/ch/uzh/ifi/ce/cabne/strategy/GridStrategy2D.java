...
public class GridStrategy2D implements Strategy<Double[], Double[]> {
    ...
    public GridStrategy2D(RealMatrix leftIntervals, RealMatrix rightIntervals, double maxV1, double maxV2) {
        ...
        this.minValue = new Double[] {0.0,0.0};
        this.leftData = leftIntervals.getData();
        this.rightData = rightIntervals.getData();
        
    }
    //for non-zero starting values/costs
    public GridStrategy2D(RealMatrix leftIntervals, RealMatrix rightIntervals, double maxV1, double maxV2,double minV1, double minV2) {
    	this.leftIntervals = leftIntervals;
        this.rightIntervals = rightIntervals;
        this.rowInterval = (maxV1-minV1) / (leftIntervals.getRowDimension() - 1);
        this.columnInterval = (maxV2-minV2) / (leftIntervals.getColumnDimension() - 1);
        this.maxValue = new Double[] {maxV1, maxV2};
        this.minValue = new Double[] {minV1, minV2};
        this.leftData = leftIntervals.getData();
        this.rightData = rightIntervals.getData();
    ...
    private double computeStrategy(double x, double y, double[][] intervals) {
        ...
        int rowIndex = (int) ((x-minValue[0]) / rowInterval);
        int columnIndex = (int) ((y-minValue[1]) / columnInterval);
    ...
    public static GridStrategy2D makeTruthful(double maxV1, double maxV2) {
    	double[][] d = new double[][]{
    		new double[]{0.0, 0.0}, //TODO change for non zero starting intervals
    		new double[]{maxV1, maxV2} //TODO check if correct
    	};
